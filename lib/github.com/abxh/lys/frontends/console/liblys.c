...
// Portability constructs based on:
...
void get_terminal_size(int *nrows, int *ncols) {
  ...
  *ncols = (int)(csbi.srWindow.Right - csbi.srWindow.Left + 1);
  *nrows = (int)(csbi.srWindow.Bottom - csbi.srWindow.Top + 1);
...
void cursor_goto(int x, int y) { printf("\033[%d;%dH", y + 1, x + 1); }
...
void keydown(struct lys_context *ctx, int keysym) {
  ...
  FUT_CHECK(ctx->fut,
            futhark_entry_key(ctx->fut, &new_state, 0, keysym, ctx->state));
...
void keyup(struct lys_context *ctx, int keysym) {
  struct futhark_opaque_state *new_state;
  FUT_CHECK(ctx->fut,
            futhark_entry_key(ctx->fut, &new_state, 1, keysym, ctx->state));
  futhark_free_opaque_state(ctx->fut, ctx->state);
  ctx->state = new_state;
}
...
void resize(struct lys_context *ctx) {
  ...
  assert(ctx->fgs && ctx->bgs && ctx->chars && ctx->rgbs);

  struct futhark_opaque_state *new_state;
  FUT_CHECK(ctx->fut, futhark_entry_resize(ctx->fut, &new_state, ctx->height,
                                           ctx->width, ctx->state));
  futhark_free_opaque_state(ctx->fut, ctx->state);
  ctx->state = new_state;
}
...
void maybe_resize(struct lys_context *ctx) {
  ...
  if (ncols != ctx->width || nrows * 2 != ctx->height) {
    resize(ctx);
  }
}
...
void lys_run_console(struct lys_context *ctx) {
  ...
  while (ctx->running && ctx->num_frames-- > 0) {
...

    {
      ...
      display(ctx->out, !ctx->interactive, nrows, ncols, ctx->fgs, ctx->bgs,
              ctx->chars);
    }
    if (ctx->interactive) {
      fflush(stdout);
...
void lys_setup(struct lys_context *ctx, int max_fps, int num_frames, FILE *out,
